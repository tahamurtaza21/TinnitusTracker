...
fun generatePdf(
    ...
    val fileName = "Tinnitus_Weekly_Report.pdf"
    ...
    // Title
    val title = Paragraph("Tinnitus Weekly Report")
        .setFontSize(20f)
        .setBold()
        .setMarginBottom(10f)
    document.add(title)
    ...
    document.add(Paragraph("Report Period: $startDate to $endDate").setItalic().setFontSize(12f).setMarginBottom(15f))
    ...
    val relaxPercent = if (totalDays > 0) report.relaxationDays.toDouble() * 100 / totalDays else 0.0
    val soundPercent = if (totalDays > 0) report.soundTherapyDays.toDouble() * 100 / totalDays else 0.0

    // Usage Stats (with percentage + raw count)
    val relaxStr = String.format("%.1f%% (%d/%d days)", relaxPercent, report.relaxationDays, totalDays)
    val soundStr = String.format("%.1f%% (%d/%d days)", soundPercent, report.soundTherapyDays, totalDays)
    ...
    document.add(Paragraph("Average Tinnitus Level: $tinnitusAvg").setFontSize(12f).setMarginBottom(10f))
    ...
    document.add(Paragraph("Average Anxiety Level: $anxietyAvg").setFontSize(12f).setMarginBottom(10f))

    // Notes
    document.add(Paragraph("\nAdditional Notes:").setBold())
    document.add(Paragraph(userNote).setItalic().setFontSize(11f).setMarginBottom(20f))
...
fun sendPdfViaEmail(context: Context, file: File, toEmail: String) {
    ...
    val intent = Intent(Intent.ACTION_SEND).apply {
        ...
        putExtra(Intent.EXTRA_SUBJECT, "Tinnitus Weekly Report")
        putExtra(Intent.EXTRA_TEXT, "Attached is the latest weekly report.")
    }
